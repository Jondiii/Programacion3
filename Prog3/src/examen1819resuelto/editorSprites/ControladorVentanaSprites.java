...
public class ControladorVentanaSprites {
	...
	public void clickBBuscar() {
		...
		void cargaFicherosGraficosOrdenados(File dir) {
			...
			int numFicherosRecorridos = cargaRec( dir, 1 );
			System.out.println( "Número de ficheros procesados: " + numFicherosRecorridos );
		}
			private int cargaRec( File dir, int nivel ) {
				if (nivel>3) return 0; // Caso base, no procesar carpetas de nivel > 3
				File[] fics = dir.listFiles(); // Ficheros o directorios de la carpeta
				Arrays.sort( fics, new Comparator<File>() {  // Ordena los ficheros primero (por nombre) y luego las carpetas
					@Override
					public int compare(File o1, File o2) {
				...
				// Primero procesamos los ficheros
				int posi = 0; // Posición de recorrido
				while (posi<fics.length && fics[posi].isFile()) {
					File f = fics[posi];
				...
				// Y luego procesamos las carpetas
				int numFicsRecs = posi;
				while (posi<fics.length) {
					numFicsRecs += cargaRec( fics[posi], nivel+1 );
					if (miVentana.mSprites.size()>=50) return numFicsRecs;  // Si ya hay 50 se corta la búsqueda
					posi++;
				}
				return numFicsRecs;
			}
